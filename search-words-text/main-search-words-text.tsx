...
const TextCheckComponent: React.FC = () => {
    ...
    const formData = new FormData();
        ...
        try {
            const response = await fetch("https://dev-magick-api.ru/php/check-text/check-text.php", {
            method: "POST",
            body: formData,
            });
        ...
        return (
            <div className="main-text-check-container">
                <h1 className="main-text-check-container-text-h1">
                    Дает возможность определить, содержится ли слово из списка в тексте загруженного файла
                </h1>
            ...
            <div className="words-change-result">
                ...
                <div className="words-change-result-text">
                    {result.split("\n\n").map((block, index) => (
                        <p key={index}>{block}</p>
                    ))}
                </div>
