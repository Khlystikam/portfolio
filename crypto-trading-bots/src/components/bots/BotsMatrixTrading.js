...
const BotsMatrixTrading = ({ data, activeTime, setActiveBot, setTimeLineCostData, setSource }) => {
    ...
    const handleBotClick = (index) => {
        if (index !== 1){
            ...
            localStorage.setItem("activeBot", index);
        }
    };
            ...
            {data.bots.map((bot, index) => {
                ...
                return (
                    ...
                    >
                        ...
                        <div className={`${result > 0 ? "bots-matrix-trading-result-plus" : "bots-matrix-trading-result-minus"} ${index === 1 ? "bots-matrix-trading-result_no_active" : ""}`}>
                            { result }%
                        </div>
                    </button>
                );
